 ...
 *  재배치하자고 했다, (같은 열, 행에 탱크는 1대만)
 *
 *  최소한의 움직임으로 배치하고자 한다.
 *
 *  점심먹기 전 재배치 하기 위한 최소의 움직임이 몇번인지 계산하는 프로그램을 구현하라
 *
 *  - Input -
...
fun findMove(N:Int, tanks:List<Pair<Int, Pair<Int, Int>>>, isRow:Boolean): MutableList<Pair<Int, Char>>{
    ...
    movingUpLeft.forEach{i ->
        // i-1번째 탱크의 현재 위치 - i 만큼 이동
        repeat(tanks[i - 1].second.first - i){
            result.add(i to (if(isRow) 'U' else 'L'))
        }
    }

    movingDownRight.sortedDescending().forEach{i ->
        // i - i-1 탱크의 현재 위치만큼 이동
...
fun main(){
    ...
    /*구현*/
    // x축을 기준으로 정렬 -> 상 하 움직일거 찾기 위함
    val tanksRowOrder = tanks.sortedBy { it.second.first }
    // 위에서부터 순서대로 위에서부터 i번째 칸으로 상,하로 움직인다.
    val rowResult = findMove(N, tanksRowOrder, true)
    ...
    val colResult = findMove(N, tanksColOrder, false)

    val answer = rowResult + colResult
    println(answer.size)
    answer.forEach{
